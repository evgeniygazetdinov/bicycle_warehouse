            ...
            for cart_item in range(len(cart_values_from_table)):
                for cart_array_item in cart_array:
                    if "Aрт" in cart_values_from_table[cart_item]:
                        if cart_values_from_table[cart_item]['название'] == cart_array_item['Название']:
                            values.append('((SELECT MAX(id)from basket)+1,{},{},{},{},"{}",{},"{}","","{}")'.format(cart_values_from_table[cart_item]['цена'],
                                    cart_values_from_table[cart_item]['кол-во'], cart_values_from_table[cart_item]['сумма'], cart_array[cart_item]['Арт'], 
                                    button_value, cart_array_item['Продаж'],dated, cart_array_item['Название']))
                            break
                    else:
                        values.append('((SELECT MAX(id)from basket)+1,{},{},{},"","{}",{},"{}","","{}")'.format(cart_values_from_table[cart_item]['цена'],
                        cart_values_from_table[cart_item]['кол-во'], cart_values_from_table[cart_item]['сумма'], button_value, cart_values_from_table[cart_item]['цена'],
                        dated, cart_values_from_table[cart_item]['название']))
                        break
